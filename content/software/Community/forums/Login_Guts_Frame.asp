...
Sub btn_Login_OnClick()
	...
	elseif ((RS_Logon.fields.getValue("Password") = txtbox_password.value) AND (RS_Logon.fields.getValue("Deleted") = FALSE) AND (RS_Logon.fields.getValue("AccountLocked") = FALSE)) then
		...
		' Cal.LogEvent 4,"User logged in successfully:"& cstr(txtbox_Username.value)
		' Set Cal = Nothing
	
		ConferenceID = Request.QueryString("ConferenceID")
		PostType = Request.QueryString("PostType")
		MessageTitle = Request.QueryString("MessageTitle")
	    ParentMessageID = Request.QueryString("ParentMessageID")
	    
	    UserID = RS_Logon.fields.getValue("UserID")
	    Session.Value("FUserID") = UserID
	   
		Response.Redirect "Post.asp?ConferenceID=" & ConferenceID & "&PostType=" & PostType & "&MessageTitle=" & MessageTitle & "&ParentMessageID=" & ParentMessageID & "&UserID=" & UserID
...
<%  if (Request.QueryString("mode") = 1) then
	 Page_Title = "<p>Authenticate for Message Post.</p>"
...
<p>
<font face=verdana>
<% if Session.Value("FUserID") <> NULL then
	  Response.Redirect "Post.asp?ConferenceID=" & ConferenceID & "&PostType=" & PostType & "&MessageTitle=" & MessageTitle & "&ParentMessageID=" & ParentMessageID & "&UserID=" & Session.Value("FUserID")
   end if
%>
You must have a valid Login ID to post messages to this forum.  
<!--METADATA TYPE="DesignerControl" startspan
